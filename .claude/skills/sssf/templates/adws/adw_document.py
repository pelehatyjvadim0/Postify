...
# ///
"""ADW Document — описать только что выполненную работу по diff.
...
Этапы: engineer(request) -> code(changes) -> documenter

Этот workflow запускается ПОСЛЕ сборки, а защита здесь структурная, а не
рекомендательная: захват изменений — этап кода, и пустой diff вызывает ошибку
ещё до запуска документатора. Пока ничего не создано, документировать нечего;
этап сообщает об этом вместо того, чтобы поручать агенту выяснять очевидное.

Под «последними изменениями» здесь понимается `git diff` относительно `--base`
(по умолчанию main); как определяется базовый коммит в ветке, в main и в чистом
дереве сразу после коммита цепочки, см. в adw_modules/changes.py.
"""
