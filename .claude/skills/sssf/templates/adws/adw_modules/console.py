"""Консольный репортёр: один рассказ, два назначения.

Каждая строка, которую печатает ADW, ТАКЖЕ попадает в БД как событие `log`,
поэтому UI с дорожками читает ту же историю, что и терминал. Обе проходят через
`_emit`, и печать с трассировкой не могут разойтись. Только обычные
последовательные строки: без спиннеров и динамических отображений, чтобы лог CI
читался точно как терминал.
"""
...
MAX_LINE = 160          # динамический текст (сводки, нарушения, ошибки) обрезается
...
class Console:
    """Привязан к tracer одного запуска. Везде доступен как `run.console`."""

    def __init__(self, tracer, adw_id: str):
        self.tracer = tracer
        self.adw_id = adw_id
        self.phase_id = ""          # текущая дорожка: к ней прикрепляются события log
        self.phase_name = ""
        self.results: list[str] = []            # статусы фаз для сводки
        self._finished = False                  # панель сводки печатается один раз
        self._out = RichConsole(highlight=False, soft_wrap=True)

    # ── единственная вспомогательная функция: всегда и печать, и трассировка ─
    ...
    def note(self, message: str) -> None:
        """Произвольная деталь текущей фазы — то, что записал `ph.log()`."""
    ...
    def gate_result(self, name: str, report) -> None:
        """Гейт сообщает, ЧТО он проверил, а не только прошёл ли он проверку."""
        ok = report.passed
