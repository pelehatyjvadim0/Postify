"""Загрузка и проверка конфигурации, а также выполнение агентов.

Каждый ADW проверяет агентов до запуска: при неполной конфигурации ничего не
создаётся. Каждый вызов агента разбирается по конкретному типу вывода; ошибки
разбора и нарушения gate повторно запрашиваются в ТОЙ ЖЕ сессии с исправлением:
контекст сохранён, число повторов ограничено. Агент предлагает, код решает.
"""
...
JSON_FIX_ATTEMPTS = 2      # число попыток continue с исправлением некорректного JSON
...
def validate(cfg: SSSFConfig, required: list[str]) -> None:
    """Быстрая проверка: каждое обязательное имя должно разрешаться в пригодного агента."""
    problems = []
...
def execute(run, phase: Phase, call: AgentCall) -> EnvelopeBase:
    """Один вызов агента: рендер prompt -> запуск pi -> типизированный разбор -> gate -> конверт."""
    ...
    run.tracer.event(EventRecord(adw_id=run.adw_id, phase_id=phase.phase_id,
                                 ...
                                 payload={"model": agent.model, "thinking": agent.thinking,
                                          ...
                                          "tools": agent.tools,  # None = все инструменты
                                          "harness_engineering": agent.harness_engineering}))
    run.console.agent_started(agent.name, agent.model, session_id)

    # Повторы разбора и исправления gate возвращаются в ТУ ЖЕ сессию pi, поэтому
    # заполненность контекста актуальна для последней отправки; расходы устроены
    # наоборот: каждая отправка стоит денег, поэтому usage накапливается.
    ...
    def send(prompt_text: str) -> agent_pi.PiResult:
        ...
        request = PiRequest(
            ...
            session_id=session_id,
            # Абсолютные пути: их читает подпроцесс pi, запущенный в repo_root.
    ...
    # Состояние дерева до работы агента. Каждая отправка этой фазы: первый
    # prompt, повторы JSON, исправления gate — измеряется относительно одной
    # этой базовой линии.
    ...
    # Gate утверждений: нарушения возвращаются в ТУ ЖЕ сессию как исправления.
    ...
    # Права проверяются после всех отправок и до принятия конверта: агент не
    # может сообщить об успехе фазы, в которой писал в запрещённое место.
    try:
    ...
    run.tracer.event(EventRecord(adw_id=run.adw_id, phase_id=phase.phase_id,
                                 ...
                                 # Итоги фазы, а не последней отправки: фаза с
                                 # повторами оплатила каждую попытку.
...
def _as_report(result) -> GateReport:
    """Принять GateReport или устаревший gate, вернувший список нарушений."""
...
def _agent_session_id(run, agent: AgentConfig) -> str:
    ...
    if entry and entry.get("model") == agent.model:
        return entry["session_id"]           # снова войти в существующее контекстное окно
    return f"sssf-{run.adw_id}-{agent.name}-{new_id(4)}"


def _event_forwarder(run, phase: Phase, agent_name: str):
    """Одно событие tool_call на реальный вызов инструмента с точными args и результатом."""
    ...
    def forward(event: dict) -> None:
        ...
        if record is None:
            return
        # Интервал вызова передаётся через столбцы; duration_ms остаётся в
        # payload как исходное авторитетное число pi.
        run.tracer.event(EventRecord(adw_id=run.adw_id, phase_id=phase.phase_id,
...
def _parse_with_retries(run, phase: Phase, call: AgentCall, result, send):
    """Разобрать итоговый ответ по объявленному типу вывода; при ошибке
    продолжить ТУ ЖЕ сессию с исправлением (число попыток ограничено)."""
