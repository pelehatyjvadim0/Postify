...
# ///
"""ADW Plan Build Test — полная стартовая цепочка.
...
Этапы: engineer(request) -> planner -> builder -> code(test) [-> builder(fix) -> code(test) ... ограниченное число раз] -> git(commit)

Тестирование — это КОД: команда набора тестов находится в
adw_modules/quality.py, поэтому агент не тратит контекстное окно на её
повторное определение. Сбои возвращаются разработчику в конверте, и только
исчерпание цикла исправления проваливает запуск.
"""
...
def main(prompt: str, config: str = "adws/adw_sssf_config/sssf.config.yaml", adw_id: str | None = None) -> int:
    ...
    # Коммитится только протестированная работа: упавший набор тестов оставляет
    # дерево без коммита.
