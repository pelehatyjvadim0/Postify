...
# ///
"""ADW Plan Build Test Quality — полная цепочка агентов и детерминированная проверка качества.
...
Этапы: engineer(request) -> planner -> builder -> [code(verify) -> code(test) -> builder(fix)] ограниченное число раз -> git(commit)

Проверка и тестирование — это КОД, а не агенты. Их команды известны, поэтому
для запуска не требуется суждение — оно нужно только для исправления. Упавший
блок не проваливает свой этап: исполнитель сделал свою работу, а не прошёл код.
Сбой становится конвертом и возвращается разработчику; запуск проваливается
только после исчерпания цикла исправления.
"""
...
def main(prompt: str, config: str = "adws/adw_sssf_config/sssf.config.yaml", adw_id: str | None = None) -> int:
    ...
    for i in range(1, MAX_FIX_LOOPS + 1):
        ...
        # run_quality() уже включает блок тестов; репозиторий, которому нужны
        # тесты в отдельном этапе, может выделить их, как показано здесь.
        ...
        # Упавший блок становится спецификацией для разработчика: дословный
        # вывод команды без парсера между сбоем и исправлением.
