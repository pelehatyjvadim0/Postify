...
from tests.unit.web.test_api import ApiClient, ApiStub, AuthStub


def test_packaged_static_mount_serves_root_and_assets(monkeypatch, tmp_path) -> None:
    # Поломка: собранный фронтенд упакован, но из приложения доступен только index.html.
    ...
    client = ApiClient(
        create_app(WebContainer(api=ApiStub()), auth=AuthStub())
    )
