...
# Bound decoded pixels before thumbnail conversion allocates image buffers.
...
def _inspect(data: bytes) -> tuple[str, int, int]:
    ...
    try:
        ...
        raise UnsupportedImage("Изображение слишком большого разрешения") from None
    except Exception:
    ...
    if width * height > MAX_IMAGE_PIXELS:
        raise UnsupportedImage(
            f"Разрешение изображения не должно превышать {MAX_IMAGE_PIXELS} пикселей"
        )
