...
class ProcessContent:
    ...
    def execute(self):
        ...
        attempts = self.r.claim(now=now, day=day, limits=self.limits)
        articles = {}
        retry = failed = 0
        for x in attempts:
            try:
                ...
                if x.attempt_no == 1:
                    ...
                    failed += 1
        if not articles:
            return ProcessContentResult(len(attempts), retry, failed, 0)
        inputs = tuple(
        ...
        try:
            ...
            return ProcessContentResult(len(attempts), retry, failed + len(articles), 0)
        ...
        for d in drafts:
            try:
                ...
                failed += 1
        return ProcessContentResult(len(attempts), retry, failed, made)
