    ...
    def claim(self, *, now, day, limits):
        with self.sf() as s:
            try:
                ...
                retry = s.execute(
                    text(
                        "SELECT a.id,a.candidate_id,2,a.source_url,c.title FROM content_attempts a JOIN candidates c ON c.id=a.candidate_id AND c.project_id=a.project_id WHERE a.project_id=:project AND a.status='retry_scheduled' AND a.retry_at<=:n ORDER BY a.retry_at FOR UPDATE SKIP LOCKED"
                    ),
                    {"project": self.project_id, "n": now},
                ).all()
                rows = []
                for r in retry:
                    ...
                    s.execute(
                        text(
                            "INSERT INTO content_attempts(project_id,candidate_id,attempt_no,tier,status,source_url,started_at) VALUES (:project,:c,2,:t,'processing',:u,:n)"
                        ),
                ...
                while len(rows) < slots and (fresh or reserve):
                    ...
                    s.execute(
                        text(
                            "INSERT INTO content_attempts(project_id,candidate_id,attempt_no,tier,status,source_url,started_at) VALUES (:project,:c,1,:t,'processing',:u,:n)"
                        ),
                ...
                s.execute(
                    text(
                        "UPDATE content_daily_usage SET analyses_started=analyses_started+:n WHERE project_id=:project AND day=:d"
                    ),
                ...
                ids = s.execute(
                    text(
                        "SELECT id,candidate_id,attempt_no,tier,source_url FROM content_attempts WHERE project_id=:project AND started_at=:n ORDER BY id"
                    ),
                    {"project": self.project_id, "n": now},
                ).all()
                s.commit()
    ...
    def save_analysis_and_create_packages(
        ...
        with self.sf() as s:
            try:
                ...
                s.execute(
                    text(
                        "UPDATE content_daily_usage SET packages_created=packages_created+:n WHERE project_id=:project AND day=:d"
                    ),
